...
class PybarsysPreferences:
    ...
    class EMAIL:
        ...
        TEMPLATE_DIR = env("PYBARSYS_EMAIL_TEMPLATE_DIR", default="email")

        # Subject of an invoice mail
        INVOICE_SUBJECT = env(
            "PYBARSYS_EMAIL_INVOICE_SUBJECT", default="Invoice from Barsys bar"
        )

        # Subject of a purchase notification to dependants
        PURCHASE_NOTIFICATION_SUBJECT = env(
            "PYBARSYS_EMAIL_PURCHASE_NOTIFICATION_SUBJECT",
            default="Purchase notification from Barsys bar",
        )

        # Subject of a payment reminder mail
        PAYMENT_REMINDER_SUBJECT = env(
            "PYBARSYS_EMAIL_PAYMENT_REMINDER_SUBJECT",
            default="Payment reminder from Barsys bar",
        )

        # Bar contact email address
        CONTACT_EMAIL = env("PYBARSYS_EMAIL_CONTACT_EMAIL", default="bar@example.com")

        # Name of bar in default templates
        NAME_OF_BAR = env("PYBARSYS_EMAIL_NAME_OF_BAR", default="Barsys bar")

        BANK_ACCOUNT_RECIPIENT = env(
            "PYBARSYS_EMAIL_BANK_ACCOUNT_RECIPIENT", default="Barsys bar"
        )
        BANK_ACCOUNT_NUMBER = env("PYBARSYS_EMAIL_BANK_ACCOUNT_NUMBER", default="55542")
        BANK_ACCOUNT_ROUTING_NUMBER = env(
            "PYBARSYS_EMAIL_BANK_ACCOUNT_ROUTING_NUMBER", default="2718"
        )
        BANK_ACCOUNT_BANK = env(
            "PYBARSYS_EMAIL_BANK_ACCOUNT_BANK", default="Royal Bank of Moldova"
        )
        # (Display name of the invoice recipient is always appended to payment reference)
        BANK_ACCOUNT_PAYMENT_REFERENCE = env(
            "PYBARSYS_EMAIL_BANK_ACCOUNT_PAYMENT_REFERENCE", default="Bar debts"
        )

    class Misc:
        # Number of purchases to show on user history page
        NUM_USER_PURCHASE_HISTORY = env.int(
            "PYBARSYS_MISC_NUM_USER_PURCHASE_HISTORY", default=15
        )
        # Whether to show total cost of unbilled purchases on user history page
        SUM_COST_USER_PURCHASE_HISTORY = env.bool(
            "PYBARSYS_MISC_SUM_COST_USER_PURCHASE_HISTORY", default=True
        )
        # User should transfer money if balance is below this value
        BALANCE_BELOW_TRANSFER_MONEY = Decimal(
            env("PYBARSYS_MISC_BALANCE_BELOW_TRANSFER_MONEY", default="0")
        )
        # Number of purchases to show on main page
        NUM_MAIN_LAST_PURCHASES = env.int(
            "PYBARSYS_MISC_NUM_MAIN_LAST_PURCHASES", default=5
        )
        # Number of users to show in a StatsDisplay on main page
        NUM_MAIN_USERS_IN_STATSDISPLAY = env.int(
            "PYBARSYS_MISC_NUM_MAIN_USERS_IN_STATSDISPLAY", default=5
        )
        # Whether to randomize order of StatsDisplays and show a random one first
        # (irrespective of show_by_default setting)
        SHUFFLE_STATSDISPLAY_ORDER = env.bool(
            "PYBARSYS_MISC_SHUFFLE_STATSDISPLAY_ORDER", default=False
        )
