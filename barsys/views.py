...
from .forms import *
from .models import SiteSettings, transfer_balance
...
class UserUpdateView(EasyVereinContactFetchMixin, UserIsAdminMixin, edit.UpdateView):
    ...
    def form_valid(self, form: UserUpdateForm) -> HttpResponse:
        transfer = None
        # Moving the balance and linking the dependant have to stand or fall together:
        # a transfer whose save failed afterwards would leave the payer charged for
        # someone who never became their dependant.
        with transaction.atomic():
            if getattr(form.instance, "_balance_moves_to_payer", False):
                payer = form.cleaned_data["purchases_paid_by_other"]
                # Drop the flag so the save below re-runs the model check for real,
                # against the balance the transfer has just zeroed.
                del form.instance._balance_moves_to_payer
                # form.instance already carries the new payer in memory, and
                # Payment.save() refuses payments for users who do not pay themselves -
                # so book against the unchanged row from the database.
                transfer = transfer_balance(
                    User.objects.get(pk=form.instance.pk), payer
                )

            response = super().form_valid(form)

        if transfer is not None:
            # payment_to carries the balance in the usual sign convention (negative =
            # owes), so it reads like the account balance shown everywhere else.
            _invoice, _payment_from, payment_to = transfer
            payer = self.object.purchases_paid_by_other
            messages.success(
                self.request,
                "Moved the remaining balance of {} from {} to {}. {}'s balance is now "
                "0; it is included in {}'s next invoice.".format(
                    currency(payment_to.amount),
                    self.object,
                    payer,
                    self.object,
                    payer,
...
class UserSettleBalanceView(UserIsAdminMixin, edit.FormView):
    """Write off a user's balance inside pybarsys, without sending a bill.

    The escape hatch for accounts that cannot be settled the normal way (deceased,
    unreachable, or not attached to EasyVerein). Deliberately independent of
    `EasyVereinPreferences.ACTIVE`: nothing is pushed anywhere, the money is written off.
    """

    template_name = "barsys/admin/user_settle_balance.html"
    form_class = UserSettleBalanceForm

    COMMENT_PREFIX = "Manual balance correction: "

    def dispatch(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        # UserPassesTestMixin only checks in super().dispatch(), so anything done
        # before that runs for anyone. The refusals below name users and their payers,
        # so check first.
        if not self.test_func():
            return self.handle_no_permission()
        self.target_user = get_object_or_404(User, pk=kwargs["pk"])
        if not self.target_user.pays_themselves():
            messages.error(
                request,
                "{} does not pay for their own purchases, so they have no balance "
                "to settle. Their purchases are billed to {}.".format(
                    self.target_user, self.target_user.purchases_paid_by_other
                ),
            )
            return redirect("admin_user_detail", pk=self.target_user.pk)
        preview = self.get_preview()
        if (
            preview["balance"] == 0
            and preview["open_purchases"] == 0
            and preview["open_payments"] == 0
        ):
            messages.warning(
                request,
                "Nothing to settle: {}'s balance is 0 and there are no unbilled "
                "purchases or payments.".format(self.target_user),
            )
            return redirect("admin_user_detail", pk=self.target_user.pk)
        return super().dispatch(request, *args, **kwargs)

    def get_preview(self) -> dict:
        """What the settlement would look like, for the confirmation page.

        The authoritative amount is derived by `create_settled_for_user` from the
        balance after invoicing; this only mirrors it so the admin can see it upfront.
        """
        return {
            "balance": self.target_user.account_balance(),
            "open_purchases": Purchase.objects.to_pay_by(self.target_user).sum_cost(),
            "open_payments": self.target_user.payments().unbilled().sum_amount(),
            "settlement": -self.target_user.open_balance(),
        }

    def get_context_data(self, **kwargs) -> dict:
        context = super().get_context_data(**kwargs)
        context["object"] = self.target_user
        context["easyverein_active"] = PybarsysPreferences.EasyVerein.ACTIVE
        context.update(self.get_preview())
        return context

    def form_valid(self, form: UserSettleBalanceForm) -> HttpResponse:
        reason = form.cleaned_data["reason"].strip()
        # The payment comment is a CharField, so the reason only fits abbreviated there;
        # the invoice keeps it in full.
        budget = Payment._meta.get_field("comment").max_length - len(
            self.COMMENT_PREFIX
        )
        invoice, payment = Invoice.objects.create_settled_for_user(
            self.target_user,
            self.COMMENT_PREFIX + Truncator(reason).chars(budget),
            invoice_comment=self.COMMENT_PREFIX + reason,
        )
        detail = (
            "Settled {}'s balance to 0 with a correction payment of {}. "
            "No bill was sent.".format(self.target_user, currency(payment.amount))
        )
        if PybarsysPreferences.EasyVerein.ACTIVE:
            detail += " Nothing was transferred to EasyVerein."
        messages.success(self.request, detail)
...
class InvoiceCreateView(UserIsAdminMixin, edit.FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        # Same reason as in UserSettleBalanceView: the redirect below would otherwise
        # happen before UserPassesTestMixin gets to check anything.
        if not self.test_func():
            return self.handle_no_permission()
        if PybarsysPreferences.EasyVerein.ACTIVE:
...
class UserStatisticsByAccountBalance(UserIsAdminMixin, FilterView):
    filterset_class = filters.UserFilter
...
# Marks a settlement invoice whose EasyVerein counterpart has not been confirmed yet.
# Written before the API call and overwritten once EasyVerein answers, so anything
# left carrying it settled a balance that EasyVerein never received.
EV_PUSH_PENDING_COMMENT = "EasyVerein push pending"
...
class EasyVereinInvoiceCreateView(UserIsAdminMixin, edit.FormView):
    ...
    def get_context_data(self, **kwargs) -> dict:
        ...
        # Settlement invoices whose EasyVerein counterpart was never confirmed. Their
        # balance was zeroed here but EasyVerein may never have received it, so an
        # admin has to check each one before that money is written off for good.
        context["pending_invoices"] = Invoice.objects.filter(
            comment=EV_PUSH_PENDING_COMMENT
    ...
    def form_valid(self, form: EasyVereinInvoicesCreateForm) -> HttpResponse:
        ...
        try:
            ...
            # Most likely a double submit; sending the same users twice would bill them
            # twice in EasyVerein.
            messages.error(self.request, str(e))
            return redirect("admin_easyverein_invoice_new")
...
def _ev_invoice_start_job(user_ids: list[int], comment: str) -> str:
    ...
    Raises ValueError if any of the users is already being invoiced by a running job.
    Two jobs over the same user would each read that user's balance before either
    writes, and both would then create an EasyVerein invoice for it while pybarsys
    records only one settlement - so the member gets billed twice.
    """
    job_id = uuid.uuid4().hex
    with _ev_invoice_jobs_lock:
        busy: set[int] = set()
        for job in _ev_invoice_jobs.values():
            if job["status"] == "running":
                busy.update(job["user_ids"])

        clash = busy.intersection(user_ids)
        if clash:
            raise ValueError(
                "Already creating invoices for: {}".format(
                    ", ".join(
                        User.objects.filter(pk__in=clash)
...
def _ev_create_invoice_for_user(
    ...
    EasyVerein invoice positions (signed so that positive = the user owes the bar):
      ...
      - payments not yet invoiced (-unbilled payments)
    ...
    sum_payments = user.payments().unbilled().sum_amount()  # P
    ...
    if balance == 0 and own_sum == 0 and dep_total == 0 and sum_payments == 0:
        result["ok"] = True
    ...
    if sum_payments:
        items_signed.append(
            ("Noch nicht abgerechnete Zahlungen (Pybarsys)", -sum_payments)
        )
    ...
    try:
        ...
        invoice, virtual_payment = Invoice.objects.create_settled_for_user(
            user,
            "Balance transferred to EasyVerein",
            is_virtual_payment=True,
            # Step 3 overwrites this with the EasyVerein invoice number. The pybarsys
            # side is committed before the API call, so a process restart in between
            # would leave the balance zeroed with nothing in EasyVerein - an invoice
            # still carrying this marker is exactly that orphan.
            invoice_comment=EV_PUSH_PENDING_COMMENT,
        )
        # What pybarsys hands over is exactly what it settled, so the settlement payment
        # is the EasyVerein total (= -balance + own_sum + dep_total - sum_payments). It is
        # read after settling though, so a purchase landing since the reads above would
        # leave the line items not adding up to it.
        ev_total = virtual_payment.amount
        ...
        ev_invoice = ev.invoice.create_with_items(
            ...
            # set_draft_state=True finalizes the invoice (draft state removed, PDF
            # generated); False keeps it as a draft regardless of isDraft. Either way
            # the EasyVerein API cannot send it, so a human still has to mail it out
            # from EasyVerein.
        ...
        ev_delete_error = None
        if ev_invoice is not None:
            try:
                ev.invoice.delete(ev_invoice)
            except Exception as delete_error:
                ev_delete_error = delete_error
        if virtual_payment is not None:
            virtual_payment.delete()
        if invoice is not None:
            invoice.delete()
        if ev_delete_error is not None:
            # A finalized invoice cannot be deleted through the API, so this is the
            # normal outcome once easyverein_finalize_invoices is on. The member has
            # been billed; reporting a plain "rolled back" here would invite a re-run
            # and bill them a second time.
            result["detail"] = (
                "FAILED AFTER BILLING: {}. Pybarsys was rolled back, but EasyVerein "
                "invoice {} still exists and could not be removed ({}). Delete it in "
                "EasyVerein before invoicing {} again, otherwise they are billed "
                "twice.".format(e, ev_invoice.id, ev_delete_error, user.display_name)
            )
        else:
